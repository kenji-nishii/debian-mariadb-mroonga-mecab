...
ENV MARIADB_VERSION 10.0.29-0+deb8u1
...
RUN { \
	...
	&& apt-get update \
	&& apt-get install -y \
		mariadb-server-10.0=10.0.29-0+deb8u1 mariadb-client-10.0=10.0.29-0+deb8u1 mariadb-client-core-10.0=10.0.29-0+deb8u1 mariadb-server-core-10.0=10.0.29-0+deb8u1 \
...
ADD groonga.list /etc/apt/sources.list.d/.

RUN apt-get update
RUN apt-get install -y --allow-unauthenticated groonga-keyring
RUN apt-get update
RUN apt-get install -y -V mariadb-server-10.0-mroonga
